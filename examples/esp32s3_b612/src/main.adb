...
--  no IDF).  Shows the reusable renderer (Glyph_Fonts) driving anti-aliased
--  B612 atlases at 12 / 16 / 24 px, and prints each size's flash footprint.
--
--  Atlas data lives in glue.c (main/b612_atlas.h, generated by gen_b612.py);
--  each size is an independent set of arrays, so a real program spends flash
--  only on the Font values it references.  Anti-aliasing works on the write-only
--  panel because glyphs are alpha-blended against a KNOWN background.
--
...
procedure Main is
   ...
   package GF  renames Glyph_Fonts;
   ...
   BG        : constant LCD.Color := LCD.Black;
   ...
   Y : Integer := 2;
   procedure Put (F : GF.Font; Str : String; FG : LCD.Color) is
   begin
      GF.Draw_Text (S, F, 4, Y + F.Ascent, Str, FG, BG);
      Y := Y + F.Height + 3;
   end Put;
...
begin
   ...
   Put (B612_16.Font, "16 px  The quick brown fox",   LCD.White);
   Put (B612_16.Font, "jumps over the lazy dog.",     LCD.White);
   ...
   Put (B612_16.Font, ".,:;!?@#&()[]{}+-=/ AaQqGg",  LCD.White);
   declare
