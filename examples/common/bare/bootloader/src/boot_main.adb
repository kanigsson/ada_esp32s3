...
procedure Boot_Main is
   ...
   procedure Poke (Addr, Val : Unsigned_32) is
      R : Unsigned_32 with Import, Volatile,
   ...
   --  ---- ROM / C imports ---------------------------------------------------
   function Spiflash_Read (Src : Unsigned_32; Dst : Address; Len : Unsigned_32)
     return Integer
   ...
   function In_DRAM (A : Unsigned_32) return Boolean is
     (A >= 16#3FC8_0000# and A < 16#3FD0_0000#);
   function In_IRAM (A : Unsigned_32) return Boolean is
     (A >= 16#4037_0000# and A < 16#403E_0000#);
   function In_DROM (A : Unsigned_32) return Boolean is
     (A >= 16#3C00_0000# and A < 16#3E00_0000#);
   function In_IROM (A : Unsigned_32) return Boolean is
     (A >= 16#4200_0000# and A < 16#4400_0000#);
   function MMU_Pages (V, Sz : Unsigned_32) return Unsigned_32 is
     (((V and 16#FFFF#) + Sz + 16#FFFF#) / 16#1_0000#);

   Hdr     : array (0 .. 5) of Unsigned_32;   --  24-byte image header
   Seg     : array (0 .. 1) of Unsigned_32;   --  per-segment (load_addr, len)
   Magic, Nseg, Entry_A, Pos, Load, Len : Unsigned_32;
   Drom_V, Drom_P, Drom_Sz : Unsigned_32 := 0;
   Irom_V, Irom_P, Irom_Sz : Unsigned_32 := 0;
   Ignore  : Integer;

   L_Msg : constant String :=
     "[ada-free-boot] loading app image @0x%08x" & ASCII.LF & ASCII.NUL;
   J_Msg : constant String :=
     "[ada-free-boot] jumping to app entry 0x%08x" & ASCII.LF & ASCII.NUL;
   Bad   : constant String :=
     "[ada-free-boot] bad image magic" & ASCII.LF & ASCII.NUL;
begin
   --  Disable the boot watchdogs the ROM armed (TG0 MWDT + RTC WDT).
   Poke (16#6001_F064#, 16#50D8_3AA1#);  Poke (16#6001_F048#, 0);  Poke (16#6001_F064#, 0);
   Poke (16#6000_80B0#, 16#50D8_3AA1#);  Poke (16#6000_8098#, 0);  Poke (16#6000_80B0#, 0);
   ...
   for I in 1 .. Nseg loop
      ...
      Load := Seg (0);  Len := Seg (1);
      Pos  := Pos + 8;
      if In_IRAM (Load) or In_DRAM (Load) then
         Ignore := Spiflash_Read (Pos, To_Address (Integer_Address (Load)),
                                  (Len + 3) and not 3);
      elsif In_DROM (Load) then
         Drom_V := Load;  Drom_P := Pos;  Drom_Sz := Len;
      elsif In_IROM (Load) then
         Irom_V := Load;  Irom_P := Pos;  Irom_Sz := Len;
      end if;
      Pos := Pos + Len;
   end loop;
   ...
   Ignore := Cache_Ibus_MMU_Set (0, Irom_V and not 16#FFFF#, Irom_P and not 16#FFFF#,
                                 64, MMU_Pages (Irom_V, Irom_Sz), 0);
   Ignore := Cache_Dbus_MMU_Set (0, Drom_V and not 16#FFFF#, Drom_P and not 16#FFFF#,
                                 64, MMU_Pages (Drom_V, Drom_Sz), 0);
   ...
   loop null; end loop;   --  unreachable; the app does not return
end Boot_Main;
