package body SHA256 is
   ...
   begin
      ...
      for I in 0 .. 7 loop                       --  64-bit big-endian length
      ...
      for Blk in 0 .. (Total / 64) - 1 loop
         ...
         begin
            for T in 0 .. 15 loop
               W (T) :=
                 Shift_Left  (Unsigned_32 (Msg (Base + T * 4)),     24) or
                 Shift_Left  (Unsigned_32 (Msg (Base + T * 4 + 1)), 16) or
                 Shift_Left  (Unsigned_32 (Msg (Base + T * 4 + 2)), 8)  or
                              Unsigned_32 (Msg (Base + T * 4 + 3));
            end loop;
            for T in 16 .. 63 loop
               S0 := Rotr (W (T - 15), 7) xor Rotr (W (T - 15), 18)
                       xor Shift_Right (W (T - 15), 3);
               S1 := Rotr (W (T - 2), 17) xor Rotr (W (T - 2), 19)
                       xor Shift_Right (W (T - 2), 10);
      ...
      return R : Digest do
         for I in 0 .. 7 loop                     --  big-endian output
