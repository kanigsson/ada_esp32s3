...
procedure Esp_Flash is
   ...
   type Termios is record                                            --  glibc layout
   ...
   type Pollfd is record
      Fd : Interfaces.C.int; Events, Revents : Interfaces.C.short;
   end record with Convention => C;

   function C_Open (Path : Interfaces.C.char_array; Flags : Interfaces.C.int)
                    return Interfaces.C.int with Import, Convention => C, External_Name => "open";
   function C_Close (Fd : Interfaces.C.int) return Interfaces.C.int
     with Import, Convention => C, External_Name => "close";
   function C_Read (Fd : Interfaces.C.int; Buf : System.Address; N : Interfaces.C.size_t)
                    return Interfaces.C.long with Import, Convention => C, External_Name => "read";
   function C_Write (Fd : Interfaces.C.int; Buf : System.Address; N : Interfaces.C.size_t)
                     return Interfaces.C.long with Import, Convention => C, External_Name => "write";
   function C_Ioctl (Fd : Interfaces.C.int; Req : Interfaces.C.unsigned_long; Arg : System.Address)
                     return Interfaces.C.int with Import, Convention => C, External_Name => "ioctl";
   function C_Poll (Fds : System.Address; N : Interfaces.C.unsigned_long; Timeout : Interfaces.C.int)
                    return Interfaces.C.int with Import, Convention => C, External_Name => "poll";
   ...
   function Sp_Open (Path : Interfaces.C.char_array) return Interfaces.C.int is
     (C_Open (Path, O_RDWR + O_NOCTTY + O_NONBLOCK));   --  flags are disjoint bits

   function Sp_Setup (Fd : Interfaces.C.int) return Interfaces.C.int is
      T : aliased Termios;
   begin
      if C_Tcgetattr (Fd, T'Address) /= 0 then return -1; end if;
      C_Cfmakeraw (T'Address);
      if C_Cfsetispeed (T'Address, B115200) /= 0 then null; end if;
      if C_Cfsetospeed (T'Address, B115200) /= 0 then null; end if;
      T.C_Cc (VMIN_Idx)  := 0;
      T.C_Cc (VTIME_Idx) := 0;
   ...
   begin
      if C_Ioctl (Fd, (if On then TIOCMBIS else TIOCMBIC), B'Address) /= 0 then null; end if;
   end Sp_Modem;
   procedure Sp_Dtr (Fd, On : Interfaces.C.int) is begin Sp_Modem (Fd, On /= 0, TIOCM_DTR); end;
   procedure Sp_Rts (Fd, On : Interfaces.C.int) is begin Sp_Modem (Fd, On /= 0, TIOCM_RTS); end;
   ...
   begin
      if C_Ioctl (Fd, TIOCMSET, M'Address) /= 0 then null; end if;
   end Sp_Dtr_Rts;

   function Sp_Read (Fd : Interfaces.C.int; Buf : System.Address;
                     N, Timeout_Ms : Interfaces.C.int) return Interfaces.C.int is
   ...
   function Sp_Write (Fd : Interfaces.C.int; Buf : System.Address; N : Interfaces.C.int)
                      return Interfaces.C.int is
     (Interfaces.C.int (C_Write (Fd, Buf, Interfaces.C.size_t (N))));

   procedure Sp_Close (Fd : Interfaces.C.int) is
   begin
      if C_Close (Fd) /= 0 then null; end if;
   end Sp_Close;
   ...
   Flash_Size : Unsigned_32 := Unsigned_32 (Board.Flash_Size);  --  --flash-size, default from board.ads
   type Flash_Pair is record
      Off : Unsigned_32; File_Idx : Natural;
   end record;
   ...
   begin
      if In_Pos >= In_Len then
         ...
         begin
            ...
            In_Len := Natural (R); In_Pos := 0;
         end;
      end if;
      declare B : constant Unsigned_8 := In_Buf (In_Pos); begin
         In_Pos := In_Pos + 1;
         return Integer (B);
      end;
   end Get_Byte;
   ...
   begin
      ...
      for X of Payload loop
         if X = C0 then
            Out_B (O) := DB; Out_B (O + 1) := 16#DC#; O := O + 2;
         elsif X = DB then
            Out_B (O) := DB; Out_B (O + 1) := 16#DD#; O := O + 2;
         else
            Out_B (O) := X; O := O + 1;
         end if;
      end loop;
   ...
   begin
      ...
      loop
         ...
         if B = C0 then
            ...
            B := Get_Byte (Timeout_Ms);
            if B < 0 then return -1; end if;
            if B = 16#DC# then Buf (N) := C0; else Buf (N) := DB; end if;
            N := N + 1;
         else
            Buf (N) := Unsigned_8 (B); N := N + 1;
         end if;
      end loop;
   end Slip_Read;
   ...
   function Command (Op : Unsigned_8; Data : Bytes; Chk : Unsigned_32;
                     Timeout_Ms : Integer; Val : out Unsigned_32) return Boolean is
   ...
   begin
      ...
      for Retry in 1 .. 100 loop
         declare L : constant Integer := Slip_Read (Rb, Timeout_Ms); begin
            exit when L < 0;
            if L >= 8 and then Rb (0) = 1 and then Rb (1) = Op then
               Val := Unsigned_32 (Rb (4))
   ...
   begin
      Sp_Rts (Fd, 0); Sp_Dtr (Fd, 0); Msleep (100);
      Sp_Dtr (Fd, 1); Sp_Rts (Fd, 0); Msleep (100);
      Sp_Rts (Fd, 1); Sp_Dtr (Fd, 0); Sp_Rts (Fd, 1); Msleep (100);
      Sp_Dtr (Fd, 0); Sp_Rts (Fd, 0);
      In_Len := 0; In_Pos := 0;             --  drop any boot noise
   end Reset_Usb_Jtag;
   ...
   begin
      ...
      if Ok then
         for I in 1 .. 7 loop               --  drain the extra sync replies
            declare Junk : Bytes (0 .. 255); begin
   ...
   function U32_Hex (V : Unsigned_32) return String is
      ...
      S : String (1 .. 8); X : Unsigned_32 := V;
   begin
      for I in reverse S'Range loop
         S (I) := H (Natural (X and 16#F#) + 1); X := Shift_Right (X, 4);
      end loop;
      return S;
   end U32_Hex;
   ...
   begin
      ...
      while I <= S'Last loop
         declare D : Unsigned_32; begin
            case S (I) is
               when '0' .. '9' => D := Character'Pos (S (I)) - Character'Pos ('0');
               when 'a' .. 'f' => D := Character'Pos (S (I)) - Character'Pos ('a') + 10;
               when 'A' .. 'F' => D := Character'Pos (S (I)) - Character'Pos ('A') + 10;
               when others     => exit;
            end case;
            V := V * 16 + D;
   ...
   begin
      ...
      while I <= S'Last loop
         declare C : constant Character := S (I); D : Unsigned_32; begin
            if C in '0' .. '9' then
      ...
      if I <= S'Last then
         case S (I) is
            when 'k' | 'K' => V := V * 16#400#;
            when 'm' | 'M' => V := V * 16#10_0000#;
            when 'g' | 'G' => V := V * 16#4000_0000#;
            when others    => null;
         end case;
      end if;
      return V;
   ...
   function Checksum (Data : Bytes) return Unsigned_8 is
      C : Unsigned_8 := 16#EF#;
   begin
      for X of Data loop C := C xor X; end loop;
      return C;
   end Checksum;
   ...
   function Spi_Set_Params (Flash_Size : Unsigned_32) return Boolean is
      P : Bytes (0 .. 23);                              --  <IIIIII>
   begin
      Put_U32 (P, 0,  0);                Put_U32 (P, 4,  Flash_Size);
      Put_U32 (P, 8,  16#1_0000#);       Put_U32 (P, 12, 16#1000#);
      Put_U32 (P, 16, 16#100#);          Put_U32 (P, 20, 16#FFFF#);
   ...
   function Read_File (Name : String) return Byte_Ptr is
      ...
      F : Ada.Streams.Stream_IO.File_Type; Last : Stream_Element_Offset;
   begin
      Open (F, In_File, Name);
      declare
   ...
   function Flash_File (Offset : Unsigned_32; Name : String) return Boolean is
      ...
      Hdr    : Bytes (0 .. 19);
   begin
      Put_Line ("[esp_flash] " & Name & " -> 0x" & U32_Hex (Offset)
                & " (" & Natural'Image (Sz) & " B," & Natural'Image (Blocks) & " blk)");
      Put_U32 (Hdr, 0,  Unsigned_32 (Sz));     Put_U32 (Hdr, 4, Unsigned_32 (Blocks));
      Put_U32 (Hdr, 8,  Flash_Block_Sz);       Put_U32 (Hdr, 12, Offset);
      Put_U32 (Hdr, 16, 0);                    --  not encrypted
      if not Cmd (16#02#, Hdr, 15000) then
         Put_Line (Standard_Error, "  flash_begin failed"); return False;
      end if;
      for Seq in 0 .. Blocks - 1 loop
         declare
            ...
            V   : Unsigned_32;
         begin
            for I in 0 .. N - 1 loop Blk (I) := Img (Lo + I); end loop;
            Put_U32 (Pay, 0, Flash_Block_Sz);  Put_U32 (Pay, 4, Unsigned_32 (Seq));
            Put_U32 (Pay, 8, 0);               Put_U32 (Pay, 12, 0);
            Pay (16 .. Pay'Last) := Blk;
   ...
   begin
      Sp_Dtr (Fd, 0); Sp_Rts (Fd, 1); Msleep (100); Sp_Rts (Fd, 0);
   end Hard_Reset;
   ...
   procedure Monitor_Run is
      ...
      C          : Character;
   begin
      In_Len := 0; In_Pos := 0;                    --  drop stale ROM/flash bytes
      loop
         B := Get_Byte (1000);
         if B < 0 then
            ...
            C := Character'Val (B);
            Ada.Text_IO.Put (C);
            if C = Character'Val (10) then Ada.Text_IO.Flush; end if;
            if C = Target (Target'First + Mpos) then
               Mpos := Mpos + 1;
               if Mpos = Target'Length then Ada.Text_IO.Flush; return; end if;
...
begin
   ...
   if Sp_Setup (Fd) /= 0 then
      Put_Line (Standard_Error, "serial setup failed"); Set_Exit_Status (1); return;
   end if;
   ...
   begin
      for Attempt in 1 .. 8 loop
         ...
         for S in 1 .. 6 loop
            if Sync then Connected := True; exit; end if;
            Msleep (50);
         end loop;
         exit when Connected;
      end loop;
      if not Connected then
         ...
         Sp_Close (Fd); Set_Exit_Status (1); return;
      end if;
   ...
   begin
      ...
      if Ok then Ok := Flash_Finish; end if;
      if not Ok then
