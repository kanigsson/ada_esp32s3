#!/bin/bash
# Build the I2C-loopback Ada (i2c_loopback.gpr) against the PINNED crate runtime into
...
EX="$(cd "$HERE/.." && pwd)"                 # examples/esp32s3_i2c_loopback/
...
( cd "$EX" && gprbuild -p -P i2c_loopback.gpr )
