#!/bin/bash
# Build the UART-loopback Ada (uart_loopback.gpr) against the PINNED crate runtime
# into a relocatable app_main.o that the bootloader's image links.
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"        # main/
EX="$(cd "$HERE/.." && pwd)"                 # examples/esp32s3_uart_loopback/
...
# gen_runtime builds that RTS and the gpr resolves its Runtime_Path.
...
( cd "$EX" && gprbuild -p -P uart_loopback.gpr )
