#!/bin/bash
# Build the TIMER Ada (timer_count.gpr) against the PINNED crate runtime into a
# relocatable app_main.o that the bootloader's image links.
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"        # main/
EX="$(cd "$HERE/.." && pwd)"                 # examples/esp32s3_timer_count/
...
( cd "$EX" && gprbuild -p -P timer_count.gpr )
