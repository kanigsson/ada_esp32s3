#!/bin/bash
# Build the RMT Ada (rmt_loopback.gpr) against the PINNED crate runtime into a
# relocatable app_main.o that the bootloader's image links.
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"        # main/
EX="$(cd "$HERE/.." && pwd)"                 # examples/esp32s3_rmt_loopback/
...
( cd "$EX" && gprbuild -p -P rmt_loopback.gpr )
