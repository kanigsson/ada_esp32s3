...
package body Chain_Verify with SPARK_Mode => On is
   ...
   --  the child was signed (RSA-PKCS1-SHA256/384/512, ECDSA/P-256 with SHA-256/384,
   --  or Ed25519) and requires the issuer to hold a matching key type.
   --
   --  Both buffers are Well_Formed and Indexable (the validator only calls this
   --  after a successful Parse), so every slice lies within its buffer (Slice_In)
   --  and each slice's 'Last is below Natural'Last - 1.  TBS and the signature are
   --  required non-empty up front; the RSA branches additionally require the
   --  issuer's modulus/exponent non-empty -- together that discharges the RSA
   --  verifiers' preconditions.  A certificate missing a field it needs cannot
   --  carry a valid signature, so returning False (Bad_Signature) is correct.
   ...
   is
   begin
      if X509.Length (Child.TBS) = 0 or else X509.Length (Child.Signature) = 0 then
         return False;
      end if;
      declare
         TBS : X509.Byte_Array renames
                 Child_Buf (Child.TBS.First .. Child.TBS.Last);
      ...
      begin
         case Child.Sig_Kind is
            when X509.Sig_RSA_SHA256 =>
               return Iss.Key_Kind = X509.Key_RSA
   ...
   begin
      ...
      --  Leaf: parse, check the host name and the leaf usage constraints.
      declare
      ...
      --  next certificate in the chain, which must itself be a CA.
      ...
      --  Anchor the top: its signature must verify under a pinned root key.  The
      --  loop above already proved Chain'Last valid; the guard re-establishes
      --  Well_Formed (TB, Top) for Sig_OK below.
      declare
