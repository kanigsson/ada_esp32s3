...
--
--  SPARK_Mode is On: the RSA PKCS#1 / PSS paths are proved AoRTE (their bodies and
--  the byte<->word helpers).  The ECDSA/P-256 and Ed25519 bodies are SPARK_Mode
--  (Off) -- they bottom out in the pure-Ada P256 and SPARKNaCl.Sign primitives,
--  the same out-of-scope boundary as the silicon RSA accelerator.
package Cert_Verify with SPARK_Mode => On is

   --  The byte strings are slices of a (finite) certificate buffer.  Capping every
   --  'Last just below Natural'Last lets the index arithmetic (length computation,
   --  +1 past-end, the BE<->word conversions) be carried out without overflow --
   --  exactly the X509.Indexable headroom convention.  The signed region (TBS /
   ...
   --  freshly built PKCS#1 block -- so there is no padding to mis-parse.  One entry
   --  per digest used by real CA signatures.
