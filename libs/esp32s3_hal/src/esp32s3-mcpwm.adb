...
package body ESP32S3.MCPWM is
   ...
   function Regs_Of (Unit : MCPWM_Unit) return Periph_Ref is
   ...
   begin
      case Ch is                       --  START = 0: stop at the next timer zero
         when Ch0 => R.TIMER0_CFG1.TIMER0_START := 0;
         when Ch1 => R.TIMER1_CFG1.TIMER1_START := 0;
         when Ch2 => R.TIMER2_CFG1.TIMER2_START := 0;
      end case;
   end Do_Stop;
   ...
   begin
      ...
      case Ch is
         when Ch0 => R.OPERATOR_TIMERSEL.OPERATOR0_TIMERSEL := 0;
         when Ch1 => R.OPERATOR_TIMERSEL.OPERATOR1_TIMERSEL := 1;
         when Ch2 => R.OPERATOR_TIMERSEL.OPERATOR2_TIMERSEL := 2;
      end case;
      ...
      case Ch is
         when Ch0 =>
            R.TIMER0_CFG0 := (TIMER0_PRESCALE => TIMER0_CFG0_TIMER0_PRESCALE_Field (Pre),
            ...
            if Has_B then
               R.DB0_CFG := (DB0_A_OUTBYPASS => False, DB0_B_OUTBYPASS => False,
                             DB0_FED_OUTINVERT => True, others => <>);
         ...
         when Ch1 =>
            R.TIMER1_CFG0 := (TIMER1_PRESCALE => TIMER1_CFG0_TIMER1_PRESCALE_Field (Pre),
            ...
            if Has_B then
               R.DB1_CFG := (DB1_A_OUTBYPASS => False, DB1_B_OUTBYPASS => False,
                             DB1_FED_OUTINVERT => True, others => <>);
         ...
         when Ch2 =>
            R.TIMER2_CFG0 := (TIMER2_PRESCALE => TIMER2_CFG0_TIMER2_PRESCALE_Field (Pre),
            ...
            if Has_B then
               R.DB2_CFG := (DB2_A_OUTBYPASS => False, DB2_B_OUTBYPASS => False,
                             DB2_FED_OUTINVERT => True, others => <>);
   ...
   begin
      ...
      case C.Idx is                    --  START = 2: start and run continuously
         when Ch0 => R.TIMER0_CFG1.TIMER0_START := 2;
         when Ch1 => R.TIMER1_CFG1.TIMER1_START := 2;
         when Ch2 => R.TIMER2_CFG1.TIMER2_START := 2;
      end case;
   end Start;
   ...
   begin
      ...
      case C.Idx is                    --  single atomic write of the comparator
         when Ch0 => R.CMPR0_VALUE0 := (CMPR0_A => CMPR0_VALUE0_CMPR0_A_Field (Cmp),
                                        others => <>);
         when Ch1 => R.CMPR1_VALUE0 := (CMPR1_A => CMPR1_VALUE0_CMPR1_A_Field (Cmp),
                                        others => <>);
         when Ch2 => R.CMPR2_VALUE0 := (CMPR2_A => CMPR2_VALUE0_CMPR2_A_Field (Cmp),
                                        others => <>);
      end case;
   end Set_Duty;
   ...
   begin
      ...
      Route_In (Fault_Signal (Unit, Input), Pin,
                (if Active_High then G.Pull_Down else G.Pull_Up));
      case Input is
         when Fault0 => R.FAULT_DETECT.F0_EN := True;
                        R.FAULT_DETECT.F0_POLE := Active_High;
         when Fault1 => R.FAULT_DETECT.F1_EN := True;
                        R.FAULT_DETECT.F1_POLE := Active_High;
         when Fault2 => R.FAULT_DETECT.F2_EN := True;
   ...
   begin
      ...
      case C.Idx is
         when Ch0 =>
            declare C2 : TZ0_CFG0_Register := R.TZ0_CFG0; begin
               if OST then
                  C2.TZ0_A_OST_U := TZ0_CFG0_TZ0_A_OST_U_Field (Act);
                  C2.TZ0_B_OST_U := TZ0_CFG0_TZ0_B_OST_U_Field (Act);
                  case Input is
                     when Fault0 => C2.TZ0_F0_OST := True;
                     when Fault1 => C2.TZ0_F1_OST := True;
                     when Fault2 => C2.TZ0_F2_OST := True;
                  end case;
               else
                  C2.TZ0_A_CBC_U := TZ0_CFG0_TZ0_A_CBC_U_Field (Act);
                  C2.TZ0_B_CBC_U := TZ0_CFG0_TZ0_B_CBC_U_Field (Act);
                  case Input is
                     when Fault0 => C2.TZ0_F0_CBC := True;
                     when Fault1 => C2.TZ0_F1_CBC := True;
                     when Fault2 => C2.TZ0_F2_CBC := True;
                  end case;
               end if;
               R.TZ0_CFG0 := C2;
            end;
         when Ch1 =>
            declare C2 : TZ1_CFG0_Register := R.TZ1_CFG0; begin
               if OST then
                  C2.TZ1_A_OST_U := TZ1_CFG0_TZ1_A_OST_U_Field (Act);
                  C2.TZ1_B_OST_U := TZ1_CFG0_TZ1_B_OST_U_Field (Act);
                  case Input is
                     when Fault0 => C2.TZ1_F0_OST := True;
                     when Fault1 => C2.TZ1_F1_OST := True;
                     when Fault2 => C2.TZ1_F2_OST := True;
                  end case;
               else
                  C2.TZ1_A_CBC_U := TZ1_CFG0_TZ1_A_CBC_U_Field (Act);
                  C2.TZ1_B_CBC_U := TZ1_CFG0_TZ1_B_CBC_U_Field (Act);
                  case Input is
                     when Fault0 => C2.TZ1_F0_CBC := True;
                     when Fault1 => C2.TZ1_F1_CBC := True;
                     when Fault2 => C2.TZ1_F2_CBC := True;
                  end case;
               end if;
               R.TZ1_CFG0 := C2;
            end;
         when Ch2 =>
            declare C2 : TZ2_CFG0_Register := R.TZ2_CFG0; begin
               if OST then
                  C2.TZ2_A_OST_U := TZ2_CFG0_TZ2_A_OST_U_Field (Act);
                  C2.TZ2_B_OST_U := TZ2_CFG0_TZ2_B_OST_U_Field (Act);
                  case Input is
                     when Fault0 => C2.TZ2_F0_OST := True;
                     when Fault1 => C2.TZ2_F1_OST := True;
                     when Fault2 => C2.TZ2_F2_OST := True;
                  end case;
               else
                  C2.TZ2_A_CBC_U := TZ2_CFG0_TZ2_A_CBC_U_Field (Act);
                  C2.TZ2_B_CBC_U := TZ2_CFG0_TZ2_B_CBC_U_Field (Act);
                  case Input is
                     when Fault0 => C2.TZ2_F0_CBC := True;
                     when Fault1 => C2.TZ2_F1_CBC := True;
                     when Fault2 => C2.TZ2_F2_CBC := True;
                  end case;
               end if;
               R.TZ2_CFG0 := C2;
            end;
   ...
   begin
      ...
      case C.Idx is                    --  pulse CLR_OST to release a latched trip
         when Ch0 => R.TZ0_CFG1.TZ0_CLR_OST := True;
                     R.TZ0_CFG1.TZ0_CLR_OST := False;
         when Ch1 => R.TZ1_CFG1.TZ1_CLR_OST := True;
                     R.TZ1_CFG1.TZ1_CLR_OST := False;
         when Ch2 => R.TZ2_CFG1.TZ2_CLR_OST := True;
   ...
   begin
      ...
      case C.Idx is
         when Ch0 => return R.TZ0_STATUS.TZ0_OST_ON or R.TZ0_STATUS.TZ0_CBC_ON;
         when Ch1 => return R.TZ1_STATUS.TZ1_OST_ON or R.TZ1_STATUS.TZ1_CBC_ON;
         when Ch2 => return R.TZ2_STATUS.TZ2_OST_ON or R.TZ2_STATUS.TZ2_CBC_ON;
      end case;
   end Faulted;
   ...
   function Is_Valid (Cap : Capture) return Boolean is (Cap.Held);

   procedure Release (Cap : in out Capture) is
      R : constant Periph_Ref := Regs_Of (Cap.U);
   begin
      if Cap.Held then
         case Cap.Idx is               --  disable the capture channel on release
            when Cap0 => R.CAP_CH0_CFG.CAP0_EN := False;
            when Cap1 => R.CAP_CH1_CFG.CAP1_EN := False;
            when Cap2 => R.CAP_CH2_CFG.CAP2_EN := False;
         end case;
   ...
   overriding procedure Finalize (Cap : in out Capture) is
   begin
      Release (Cap);
   end Finalize;
   ...
   is
      ...
      Mode : constant CAP_CH0_CFG_CAP0_MODE_Field :=
        (case Edge is when Rising => 2, when Falling => 1, when Both_Edges => 3);
   begin
      ...
      case Chan is
         when Cap0 => R.CAP_CH0_CFG := (CAP0_EN => True, CAP0_MODE => Mode, others => <>);
                      R.INT_CLR.CAP0_INT_CLR := True;
         when Cap1 => R.CAP_CH1_CFG := (CAP1_EN => True, CAP1_MODE => Mode, others => <>);
                      R.INT_CLR.CAP1_INT_CLR := True;
         when Cap2 => R.CAP_CH2_CFG := (CAP2_EN => True, CAP2_MODE => Mode, others => <>);
   ...
   begin
      ...
      case Cap.Idx is
         when Cap0 => return R.INT_RAW.CAP0_INT_RAW;
         when Cap1 => return R.INT_RAW.CAP1_INT_RAW;
         when Cap2 => return R.INT_RAW.CAP2_INT_RAW;
      end case;
   end Capture_Pending;
   ...
   begin
      ...
      case Cap.Idx is
         when Cap0 => Value := Unsigned_32 (R.CAP_CH0);
                      Falling := R.CAP_STATUS.CAP0_EDGE;   --  True = negedge
                      R.INT_CLR.CAP0_INT_CLR := True;
         when Cap1 => Value := Unsigned_32 (R.CAP_CH1);
                      Falling := R.CAP_STATUS.CAP1_EDGE;
                      R.INT_CLR.CAP1_INT_CLR := True;
         when Cap2 => Value := Unsigned_32 (R.CAP_CH2);
