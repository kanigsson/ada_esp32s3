...
package body ESP32S3.MAC is
   ...
   function Wi_Fi_Station return MAC_Address is (Base);
   function Wi_Fi_SoftAP  return MAC_Address is (Derived (1));
   function Bluetooth     return MAC_Address is (Derived (2));
   function Ethernet      return MAC_Address is (Derived (3));
   ...
   function Image (Addr : MAC_Address) return String is
      ...
      function H (B : Unsigned_8) return String is
        (Hex (Integer (Shift_Right (B, 4))) & Hex (Integer (B and 16#F#)));
   begin
      return H (Addr (0)) & ":" & H (Addr (1)) & ":" & H (Addr (2)) & ":"
           & H (Addr (3)) & ":" & H (Addr (4)) & ":" & H (Addr (5));
   end Image;
