...
package body Modbus.Master is
   ...
   begin
      if S.Open then
         begin Close_Socket (S.Sock); exception when others => null; end;
         S.Open := False;
      end if;
   end Close;
   ...
   begin
      ...
      Connect_Socket
        (S.Sock, (Family => Family_Inet, Addr => Inet_Addr (Host), Port => Port));
      S.Open := True;
      S.TID  := 0;
      Result := OK;
   exception
   ...
   function Recv_Exact (S : in out Session; Offset, Count : Natural)
                        return Recv_Result is
      Got : Natural := 0;
   begin
      while Got < Count loop
         ...
         begin
            ...
            exception
               when Socket_Error => return Recv_Timeout;   --  timeout or error
            end;
   ...
   ---------------------------------------------------------------------------
   procedure Transact (S         : in out Session;
                       Unit      : Unit_Id;
                       PDU_Len   : Natural;
   ...
   begin
      ...
      case Recv_Exact (S, 0, MBAP_Size) is               --  the reply MBAP
         when Recv_OK      => null;
         when Recv_Timeout => Result := Timed_Out;   return;
         when Recv_Closed  => Close (S); Result := Disconnected; return;
      end case;
      ...
      case Recv_Exact (S, MBAP_Size, Length - 1) is      --  the reply PDU
         when Recv_OK      => null;
         when Recv_Timeout => Result := Timed_Out;   return;
         when Recv_Closed  => Close (S); Result := Disconnected; return;
      end case;
   ...
   procedure Read_Bits (S : in out Session; FC : Function_Code; Unit : Unit_Id;
                        Addr : Address; Qty : Positive; Into : out Bit_Array;
                        Result : out Status; Exc : out Exception_Code) is
      RLen : Natural;
   ...
   procedure Read_Regs (S : in out Session; FC : Function_Code; Unit : Unit_Id;
                       Addr : Address; Qty : Positive; Into : out Word_Array;
                       Result : out Status; Exc : out Exception_Code) is
      RLen : Natural;
   ...
   procedure Read_Coils
     (S : in out Session; Unit : Unit_Id; Addr : Address; Qty : Positive;
      Into : out Bit_Array; Result : out Status; Exc : out Exception_Code) is
   begin
   ...
   procedure Read_Holding_Registers
     (S : in out Session; Unit : Unit_Id; Addr : Address; Qty : Positive;
      Into : out Word_Array; Result : out Status; Exc : out Exception_Code) is
   begin
   ...
   procedure Write_Single_Coil
     (S : in out Session; Unit : Unit_Id; Addr : Address; Value : Boolean;
      Result : out Status; Exc : out Exception_Code) is
      RLen : Natural;
   begin
   ...
   procedure Write_Single_Register
     (S : in out Session; Unit : Unit_Id; Addr : Address; Value : Word;
      Result : out Status; Exc : out Exception_Code) is
      RLen : Natural;
   begin
   ...
   procedure Write_Multiple_Coils
     (S : in out Session; Unit : Unit_Id; Addr : Address; Values : Bit_Array;
      Result : out Status; Exc : out Exception_Code) is
   ...
   procedure Write_Multiple_Registers
     (S : in out Session; Unit : Unit_Id; Addr : Address; Values : Word_Array;
      Result : out Status; Exc : out Exception_Code) is
      Qty  : constant Natural := Values'Length;
      Bc   : constant Natural := 2 * Qty;
      RLen : Natural;
