...
package body ESP32S3.Touch is
   ...
   function Pad (Ch : Channel) return ESP32S3.GPIO.Pin_Id is
     (ESP32S3.GPIO.Pin_Id (Natural (Ch)));
   ...
   --  counter stays at 0.
   Touch_DAC  : UInt32 with Volatile, Import, Address => RTC_CNTL_Periph.TOUCH_DAC'Address;
   Touch_DAC1 : UInt32 with Volatile, Import, Address => RTC_CNTL_Periph.TOUCH_DAC1'Address;
   ...
   begin
      ...
      RTC_CNTL_Periph.TOUCH_CTRL2 :=
        (TOUCH_DREFH => 3, TOUCH_DREFL => 0, TOUCH_DRANGE => 3,
         TOUCH_XPD_WAIT => 16#FF#, TOUCH_XPD_BIAS => True,
         TOUCH_START_FSM_EN => True, TOUCH_START_FORCE => False,   --  timer mode
         TOUCH_CLKGATE_EN => True, others => <>);
   ...
   begin
      ...
      if N < 10 then
         ...
         begin
            Touch_DAC := (Touch_DAC and not Shift_Left (UInt32 (7), Shift))
         ...
         begin
            Touch_DAC1 := (Touch_DAC1 and not Shift_Left (UInt32 (7), Shift))
                            or Shift_Left (UInt32 (Slope), Shift);
         end;
      end if;
   ...
   function Touched (Ch        : Channel;
                     Reference : Natural;
                     Margin    : Natural := 20_000) return Boolean is
