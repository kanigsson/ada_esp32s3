...
package body ESP32S3.W5500 is
      ...
      while Off <= Data'Last loop
         ...
         begin
            ...
            ESP32S3.SPI.Transfer
              (S, Scratch_Tx (H)'Address, Scratch_Rx (H)'Address, 3 + N);
   ...
   begin
      ...
      while Off <= Data'Last loop
         ...
         begin
            ...
            for I in 0 .. N - 1 loop                           --  skip the 3 header echoes
   ...
   begin
      Write (Dev, Blk, Addr,
             (Byte (Shift_Right (V, 8)), Byte (V and 16#FF#)));
   end Write_U16;
   ...
   begin
      ...
      if Rst /= ESP32S3.GPIO.No_Pin then
         ESP32S3.GPIO.Configure (ESP32S3.GPIO.Pin_Id (Rst),
                                 Mode => ESP32S3.GPIO.Output,
                                 Pull => ESP32S3.GPIO.Pull_Up);
         ESP32S3.GPIO.Set (ESP32S3.GPIO.Pin_Id (Rst));     --  RSTn high = not reset
      end if;

      if Int /= ESP32S3.GPIO.No_Pin then                   --  for the future ISR child
         ESP32S3.GPIO.Configure (ESP32S3.GPIO.Pin_Id (Int),
   ...
   begin
      if Dev.Rst /= ESP32S3.GPIO.No_Pin then
         ...
         for Tries in 1 .. 100 loop                           --  self-clears when done
   ...
   function Version (Dev : Device) return Byte is
     (Read_U8 (Dev, Common_Regs, VERSIONR));

   function Present (Dev : Device) return Boolean is
     (Version (Dev) = W5500_VER);

   procedure Configure
     (Dev                 : in out Device;
      MAC                 : MAC_Address;
      IP, Subnet, Gateway : IPv4_Address) is
   begin
   ...
   begin
      return (Link   => (if (R and PHY_LNK) /= 0 then Up else Down),
              Speed  => (if (R and PHY_SPD) /= 0 then Mbps_100 else Mbps_10),
              Duplex => (if (R and PHY_DPX) /= 0 then Full else Half));
   end Phy;

   function Link (Dev : Device) return Link_State is (Phy (Dev).Link);
