...
package body ESP32S3.UART.Engine is
   ...
   begin
      Regs.CLK_CONF :=
        (SCLK_SEL     => 3,                              --  XTAL
         ...
         RX_SCLK_EN   => True,
         others       => <>);
      Regs.CLKDIV := (CLKDIV => CLKDIV_CLKDIV_Field (Clk_Div / 16),
   ...
   begin
      ...
      Regs.CONF0 :=
        (BIT_NUM      => CONF0_BIT_NUM_Field (Natural (Bits) - 5),
         STOP_BIT_NUM => (case Stop is when One => 1, when Two => 3),
         PARITY_EN    => Parity /= None,
         PARITY       => Parity = Odd,         --  PARITY: 1 = odd, 0 = even
         RXFIFO_RST   => True,
         TXFIFO_RST   => True,
   ...
   begin
      if B.Valid then
         B.Regs.CONF0.STOP_BIT_NUM :=
           (case Stop is when One => 1, when Two => 3);
      end if;
   end Set_Stop_Bits;
   ...
   begin
      ...
      if Rts /= G.No_Pin then
         ...
         B.Regs.MEM_CONF.RX_FLOW_THRHD := MEM_CONF_RX_FLOW_THRHD_Field
   ...
   begin
      ...
      for D of Data loop
         ...
         B.Regs.FIFO := (RXFIFO_RD_BYTE => ESP32S3_Registers.Byte (D),
                         others         => <>);
      end loop;
   end Write;
   ...
   function Rx_Available (B : Bus) return Natural is
     (if B.Valid then Natural (B.Regs.STATUS.RXFIFO_CNT) else 0);
