...
package ESP32S3.Ext4.Superblock is
   ...
   function Has_Filetype (SB : Info) return Boolean is
     ((SB.Feature_Incompat and Incompat_Filetype) /= 0);

   function Is_64Bit (SB : Info) return Boolean is
     ((SB.Feature_Incompat and Incompat_64Bit) /= 0);
   ...
   function SB_Block  (Block_Size : Natural) return Block_Number is
     (if Block_Size = 1024 then 1 else 0);
   function SB_Offset (Block_Size : Natural) return Natural is
     (if Block_Size = 1024 then 0 else 1024);
