...
package body ESP32S3.Ext4.Journal is
   ...
   function Needs_Recovery (V : Volume.Context) return Boolean is
     ((V.SB.Feature_Incompat and Superblock.Incompat_Recover) /= 0);
   ...
   procedure Replay (V : in out Volume.Context) is
      ...
      function Wrap (X : U64) return U64 is
        (if X >= Maxlen then First + (X - Maxlen) else X);

      function Get16BE (B : Byte_Array; Off : Natural) return U32 is
        (Shift_Left (U32 (B (B'First + Off)), 8) or U32 (B (B'First + Off + 1)));

      Max_Rev : constant := 256;
      type Rev_Rec is record Blk, Seq : U64; end record;
      ...
      begin
         while not Last and then Pos + 8 <= BS loop
            ...
            begin
               N   := N + 1;
               Pos := Pos + 8;
               if Use_64 then Pos := Pos + 4; end if;
               if (Flags and FL_Same_UUID) = 0 then Pos := Pos + 16; end if;
      ...
      if Start /= 0 then
         ...
         begin
            Scan : loop
               ...
               RJ (Cur, Meta.all);
               exit Scan when Get_U32_BE (Meta.all, 0) /= Magic
                     ...
                     begin
                        while Pos + RSz <= Count loop
                           declare
                              B : constant U64 :=
                                (if Use_64
                                 then Shift_Left (U64 (Get_U32_BE (Meta.all, Pos)), 32)
         ...
         begin
            Rep : loop
               ...
               RJ (Cur, Meta.all);
               exit Rep when Get_U32_BE (Meta.all, 0) /= Magic
                 or else Get_U32_BE (Meta.all, 8) /= U32 (Seq and 16#FFFF_FFFF#);
               declare
                  BType : constant U32 := Get_U32_BE (Meta.all, 4);
               begin
                           ...
                           begin
                              ...
                              begin
                                 ...
                                 if not Revoked (Target, Seq) then
                                    Block_Cache.Write
                                      (V.Cache, Block_Number (Target), Data.all);
                                 end if;
   ...
   is
      ...
      function Jphys (Jrel : U64) return Block_Number is
      ...
      begin
         for I in 0 .. N - 1 loop
            declare
               ...
               Flags : U32 := 0;
            begin
               if I = N - 1 then Flags := Flags or FL_Last; end if;
               if I > 0      then Flags := Flags or FL_Same_UUID; end if;
               if Esc        then Flags := Flags or FL_Escape; end if;
               Put_U32_BE (Blk.all, Pos, U32 (Targets (Targets'First + I)));
               Blk.all (Pos + 6) := 0;                      --  t_flags (be16)
               Blk.all (Pos + 7) := U8 (Flags and 16#FF#);
               Pos := Pos + 8;
               if Use_64 then
                  Put_U32_BE (Blk.all, Pos,
                              U32 (Shift_Right (U64 (Targets (Targets'First + I)), 32)));
                  Pos := Pos + 4;
               end if;
               if I = 0 then
   ...
   is
      ...
      begin
         if N < Max_WS - 1 then       --  leave room for the SB target
            N := N + 1;
            Targets (N) := B;
         end if;
      end Collect;
      ...
      begin
         ...
         ESP32S3.Block_Dev.Read_Sector (V.Dev, 2, Sec); Buf (0 .. 511)   := Byte_Array (Sec);
         ESP32S3.Block_Dev.Read_Sector (V.Dev, 3, Sec); Buf (512 .. 1023) := Byte_Array (Sec);
         Superblock.Encode (Tmp, Buf, 0);
         Sec := ESP32S3.Block_Dev.Sector (Buf (0 .. 511));   ESP32S3.Block_Dev.Write_Sector (V.Dev, 2, Sec);
         Sec := ESP32S3.Block_Dev.Sector (Buf (512 .. 1023)); ESP32S3.Block_Dev.Write_Sector (V.Dev, 3, Sec);
      end Write_SB_Raw;
   ...
   begin
      ...
      begin
         for I in 1 .. N loop
            ...
            declare
               Flags : U32 := 0;
            begin
               if I = N then Flags := Flags or FL_Last; end if;
               if I > 1  then Flags := Flags or FL_Same_UUID; end if;
               if Esc    then Flags := Flags or FL_Escape; end if;
               ...
               if Use_64 then
                  Put_U32_BE (Blk.all, Pos,
                              U32 (Shift_Right (U64 (Targets (I)), 32)));
                  Pos := Pos + 4;
               end if;
               if I = 1 then
                  Blk.all (Pos .. Pos + 15) := UUID;
      ...
      if Simulate_Crash then           --  stop -- metadata NOT yet checkpointed
         Free (Jsb); Free (Blk); Free (SBblk);
         return;
      end if;
