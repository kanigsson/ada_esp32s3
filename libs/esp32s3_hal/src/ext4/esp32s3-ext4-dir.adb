...
package body ESP32S3.Ext4.Dir is
   ...
   generic
      with function Visit (Name : String; Ino : Inode_Number; File_Type : U8)
                           return Boolean;
      ...
      for LB in 0 .. N_Blk - 1 loop
         declare
            Phys : constant Block_Number :=
                     Block_Map.Logical_To_Physical (V, Dir, LB);
            Pos  : Natural := 0;
         begin
            if Phys /= 0 then
               while Pos + 8 <= BS loop
   ...
   procedure Add_Entry
     (V         : in out Volume.Context;
      ...
      File_Type : U8)
   is
      function R4 (N : Natural) return Natural is ((N + 3) / 4 * 4);
      BS     : constant Natural := V.SB.Block_Size;
      N_Blk  : constant U64     := (Dir.Size + U64 (BS) - 1) / U64 (BS);
                  ...
                  begin
                     ...
                     if Ino /= 0 and then NLen = Name'Length then
                        ...
                        if To_String (Nm) = Name then
                           if Prev_Pos >= 0 then        --  merge into previous
                              ESP32S3.Ext4.Block_Cache.Read_At
                                (V.Cache, Phys, Prev_Pos, PHdr);
                              Put_U16 (PHdr, 4,
                                       U16 (Natural (Get_U16 (PHdr, 4)) + Rec));
                              ESP32S3.Ext4.Block_Cache.Write_At
                                (V.Cache, Phys, Prev_Pos, PHdr);
                           else                          --  first: zero its inode
                              Put_U32 (Hdr, 0, 0);
                              ESP32S3.Ext4.Block_Cache.Write_At
                                (V.Cache, Phys, Pos, Hdr);
                           end if;
