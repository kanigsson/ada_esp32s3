...
package body ESP32S3.GPIO is
   ...
   function Lo_Bit (Pin : Pin_Id) return UInt32 is
     (UInt32'(2) ** Natural (Pin))                    with Inline;     --  pins 0..31
   function Hi_Bit (Pin : Pin_Id) return UInt22 is
     (UInt22'(2) ** (Natural (Pin) - 32))             with Inline;     --  pins 32..48
   ...
   protected body Lock is
      ...
      begin
         ...
         declare
            O : Reg.FUNC_OUT_SEL_CFG_Register :=
   ...
   begin
      if On then Set (Pin); else Clear (Pin); end if;
   end Write;
