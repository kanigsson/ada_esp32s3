...
package body ESP32S3.RMT is
   ...
   function Div_Of (Resolution_Hz : Positive) return Byte is
     (Byte (Natural'Max (1, Natural'Min (255, Src_Hz / Resolution_Hz))));
   ...
   protected body Pool is
      ...
      begin
         ...
         RMT_Periph.SYS_CONF := (SCLK_SEL => 1, SCLK_DIV_NUM => 0,
                                 SCLK_ACTIVE => True, MEM_CLK_FORCE_ON => True,
                                 APB_FIFO_MASK => True,
                                 CLK_EN => True, others => <>);
         Inited := True;
      end Init;
   ...
   function TX_Done (Idx : TX_Index) return Boolean is
     (case Idx is when 0 => RMT_Periph.INT_RAW.CH0_TX_END,
   ...
   begin
      case Idx is
         when 0 => RMT_Periph.INT_CLR.CH0_TX_END := True;
         when 1 => RMT_Periph.INT_CLR.CH1_TX_END := True;
         when 2 => RMT_Periph.INT_CLR.CH2_TX_END := True;
         when 3 => RMT_Periph.INT_CLR.CH3_TX_END := True;
      end case;
   end Clear_TX_Done;
   ...
   begin
      case Idx is
         when 0 => RMT_Periph.INT_CLR.CH4_RX_END := True;
         when 1 => RMT_Periph.INT_CLR.CH5_RX_END := True;
         when 2 => RMT_Periph.INT_CLR.CH6_RX_END := True;
         when 3 => RMT_Periph.INT_CLR.CH7_RX_END := True;
      end case;
   end Clear_RX_Done;
   ...
   function Is_Valid (C : TX_Channel) return Boolean is (C.Held);
   ...
   overriding procedure Finalize (C : in out TX_Channel) is
   begin
      Release (C);
   end Finalize;
   ...
   function Is_Valid (C : RX_Channel) return Boolean is (C.Held);
   ...
   overriding procedure Finalize (C : in out RX_Channel) is
   begin
      Release (C);
   end Finalize;
   ...
   begin
      ...
      RX_Conf (C.Idx).CONF0 :=
        (DIV_CNT => Div_Of (Resolution_Hz),
         ...
         MEM_SIZE => 1, CARRIER_EN => False, others => <>);
      RX_Conf (C.Idx).CONF1 :=
        (MEM_OWNER => True, RX_FILTER_EN => True, RX_FILTER_THRES => 10,
         RX_EN => False, others => <>);
   ...
   begin
      ...
      begin
         ...
         if Guard = 0 then          --  timed out: nothing received
            return;
         end if;
      end;
