...
package body ESP32S3.AES is
   ...
   protected Engine is
      procedure Transform (Mode : UInt32; Key : Key_Bytes;
                           Input : Block; Output : out Block);
   private
   ...
   protected body Engine is
      ...
      begin
         ...
         for I in 0 .. Key'Length / 4 - 1 loop
            AES_Periph.KEY (I) :=
              To_Word (Key (Key'First + I * 4),     Key (Key'First + I * 4 + 1),
                       Key (Key'First + I * 4 + 2), Key (Key'First + I * 4 + 3));
         end loop;

         for I in 0 .. 3 loop
            AES_Periph.TEXT_IN (I) := To_Word (Input (I * 4),     Input (I * 4 + 1),
   ...
   function Enc_Mode (Len : Natural) return UInt32 is
     (if Len = 32 then Encrypt_256 else Encrypt_128);
   function Dec_Mode (Len : Natural) return UInt32 is
     (if Len = 32 then Decrypt_256 else Decrypt_128);
