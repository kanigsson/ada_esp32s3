...
\textbf{Field} & \textbf{Off} & \textbf{Meaning} \\ \hline
\endhead
...
\section{The windowed-ABI premise}
...
\section{Starting a task that has never run}
...
\section{What state is saved \textemdash{} and what is not}

\begin{longtable}{p{5.5cm} p{2.2cm} p{5.5cm}}
\textbf{State} & \textbf{Saved?} & \textbf{Notes} \\ \hline
\endhead
...
The FPU is handled \emph{eagerly per task}: \texttt{CPENABLE} is enabled globally,
every task is given a \texttt{CP\_State} save area, and the switch copies all
sixteen \texttt{F} registers plus \texttt{FCR}/\texttt{FSR} whenever the buffer's
\texttt{CP\_State} pointer is non-null. \texttt{THREADPTR} was the last addition to
