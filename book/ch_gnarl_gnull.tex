...
-- comes later, in \S\ref{ch:tasking}; this chapter is the runtime underneath
them: who is called, what each layer is responsible for, and which real source
units implement it.

\section{Two names, two layers}\index{GNARL}\index{GNULL}\index{tasking runtime}
...
\section{The layer cake}\index{bare-board kernel}
...
\section{GNARL's upper layer: the ATCB}\index{task control block}\index{ATCB}
...
\section{The GNULL primitive set}\index{task primitives}
...
\subsection{Where the context switch lives}\index{context switch}
...
\section{The thread descriptor: GNULL's view of a task}\index{thread descriptor}\index{scheduler}
...
\section{Three profiles, one boundary}\index{runtime profile}\index{Ravenscar}\index{protected object}
...
\section{Following one operation all the way down}\index{delay until}
