...
\texttt{Setup} brings a host up and claims a GDMA channel; mode and clock are not
set here, because on a shared bus they are a per-\emph{device} property, applied at
\texttt{Acquire}. \texttt{Configure\_Pins} routes the shared
...
one pad for a self-test. \texttt{Acquire} takes the host and, under the exclusive
hold, programs that device's \texttt{Mode} (CPOL/CPHA 0\,..\,3) and \texttt{Clock\_Hz}
(roughly 80\,kHz\,..\,80\,MHz) -- so a flash at mode 0 / 8\,MHz and a display at
mode 3 / 40\,MHz can share one controller, and an SD card can negotiate at
$\le$400\,kHz then switch to several MHz on its next \texttt{Acquire}.
\texttt{Set\_Clock} re-tunes the rate within a single hold (no DMA re-claim), for a
device that changes speed mid-transaction. The single transfer primitive is a
blocking, full-duplex DMA \texttt{Transfer} of 1\,..\,4095 bytes between two SRAM
buffers, addressed by \texttt{System.Address}. A device that brings its own
chip-select -- to share the bus -- names its select pin as \texttt{CS\_Pin} at
\texttt{Acquire} (the driver owns the GPIO, active-low, held across the
transaction) and brackets its transaction with \texttt{Select\_Device}; the driver
then suppresses the hardware \texttt{CS0} for that session. A select that is not one
plain GPIO (behind a 3:8 decoder or an I/O-expander) passes a library-level
\texttt{CS\_Select} callback instead. The controlled \texttt{Session} auto-releases
on scope exit, so this driver targets \texttt{embedded}/\texttt{full}.
...
package ESP32S3.SPI is
   ...
   procedure Set_Clock   (Host : SPI_Host; Hz : Positive);   --  mid-hold re-tune
   ...
   procedure Configure_Pins (Host : SPI_Host;   --  the shared bus wires
...
begin
   ...
   Acquire (S, SPI2);                          --  mode 0, default clock
...
\section{MAC -- Factory Ethernet Address}\index{MAC driver}\index{MAC address}\index{eFuse}
\label{app:int:mac}

\texttt{ESP32S3.MAC} reads the MAC address Espressif programs into every part at
the factory. The chip is allocated a \emph{block of four} universally-administered
addresses: the eFuse base is the Wi-Fi station MAC, and soft-AP, Bluetooth and
Ethernet are base $+\,1$, $+\,2$ and $+\,3$. The values live in eFuse shadow
registers (\texttt{RD\_MAC\_SPI\_SYS\_0}/\texttt{\_1}) that the hardware latches at
reset, so every routine is a \emph{pure read} -- no clock to gate, no controller to
enable, no shared state and no protected object. Like \texttt{ESP32S3.RNG} it
compiles in every runtime profile, ZFP included.

The intended use is handing a W5500 a real, manufacturer-assigned address instead
of a hand-picked one that might collide on the network (Chapter~\ref{ch:networking},
\S\ref{ch:net:mac}): pass \texttt{Ethernet} to the driver's \texttt{Configure}. A
board with two W5500s has only the one Ethernet slot, so the second NIC takes a
\emph{locally-administered} address -- \texttt{Local (Derived (4))}, base $+\,4$
with the local bit set -- which is guaranteed not to clash with a registered one.
\texttt{MAC\_Address} is six bytes most-significant-first (\texttt{MAC (0)} is the
OUI's first byte, the order the W5500 and DHCP expect).

\begin{lstlisting}
package ESP32S3.MAC is
   type MAC_Address is array (0 .. 5) of Interfaces.Unsigned_8;

   function Base          return MAC_Address;   --  factory base (= Wi-Fi station)
   ...
   function Ethernet      return MAC_Address;   --  base + 3  (use for a W5500)

   --  Base with Offset added to its last byte; 0 .. 3 are the universal MACs above.
   function Derived (Offset : Interfaces.Unsigned_8) return MAC_Address;
   --  Mark a MAC locally-administered + unicast (for addresses you assign).
   function Local   (Addr   : MAC_Address)           return MAC_Address;
   function Image   (Addr   : MAC_Address)           return String;  --  "aa:bb:..:ff"
end ESP32S3.MAC;
\end{lstlisting}

A unique Ethernet MAC for one NIC, and a safe locally-administered one for a second:

\begin{lstlisting}
with ESP32S3.MAC; use ESP32S3;

Eth0 : constant MAC.MAC_Address := MAC.Ethernet;             --  base + 3, universal
Eth1 : constant MAC.MAC_Address := MAC.Local (MAC.Derived (4)); -- base + 4, local bit
begin
   --  W5500.Configure (Dev0, MAC => To_W5500 (Eth0), ...);   --  unique per chip
   Put_Line (MAC.Image (Eth0));                               --  e.g. "7c:df:a1:..:..:.."
\end{lstlisting}

\noindent\textbf{Example:} \texttt{examples/esp32s3\_mac} prints all five (base, the
three derived interface MACs, and a local one); \texttt{esp32s3\_multinic} seeds its
two W5500s this way.
