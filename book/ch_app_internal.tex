...
\section{UART --- Asynchronous Serial}
\label{app:int:uart}
...
addressed by \texttt{System.Address}. The controlled \texttt{Session}
auto-releases on scope exit, so this driver targets \texttt{embedded}/\texttt{full}.
...
\section{RNG --- Hardware Random Number Generator}
\label{app:int:rng}
...
\section{Temperature --- On-Chip Die Sensor}
...
\section{LEDC --- LED Controller (PWM)}
\label{app:int:ledc}
...
\section{MCPWM --- Motor-Control PWM}
\label{app:int:mcpwm}
...
\section{Timer --- General-Purpose Timers}
\label{app:int:timer}
...
\section{SDM --- Sigma-Delta Modulator}
\label{app:int:sdm}
...
\section{RMT --- Remote Control Transceiver}
\label{app:int:rmt}
...
\section{I2S --- Digital Audio Serial Bus}
\label{app:int:i2s}
...
\section{LCD --- i8080 Parallel Display}
\label{app:int:lcd}
...
\section{ADC --- SAR Analog-to-Digital Converter}
\label{app:int:adc}
...
\section{RTC --- Real-Time Clock and Deep Sleep}
\label{app:int:rtc}
...
\section{RTC-IO --- Low-Power GPIO and Pad Hold}
\label{app:int:rtcio}
...
\section{Touch --- Capacitive Touch Sensor}
\label{app:int:touch}
...
\section{SHA --- Hardware Hash Accelerator}
\label{app:int:sha}
...
\section{AES --- Hardware AES Accelerator}
\label{app:int:aes}
...
\section{SDMMC --- Native SD/MMC Host Controller}
\label{app:int:sdmmc}
