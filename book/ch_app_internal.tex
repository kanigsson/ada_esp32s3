...
to share the bus --- names its select pin as \texttt{CS\_Pin} at \texttt{Acquire}
(the driver owns the GPIO, active-low, held across the transaction) and brackets
its transaction with \texttt{Select\_Device}; the driver then suppresses the
hardware \texttt{CS0} for that session. A select that is not one plain GPIO (behind
a 3:8 decoder or an I/O-expander) passes a library-level \texttt{CS\_Select}
callback instead. The controlled \texttt{Session} auto-releases on scope exit, so
this driver targets \texttt{embedded}/\texttt{full}.
