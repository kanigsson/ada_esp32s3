...
\item[RAII.] Those \texttt{Session}/\texttt{Channel}/\texttt{Reader} handles are
  ...
  The lock-free drivers (GPIO, RNG, Temperature, RTC, RTC\_IO, Touch, SHA, AES,
  SDMMC) work under every profile.
...
over GDMA. Configure the host once, then \texttt{Acquire} a \texttt{Session} for a
transaction. \texttt{Transfer} shifts \texttt{Tx} out while capturing \texttt{Rx}.
...
\item[\texttt{Mode}] SPI clock mode \texttt{0\,..\,3} (the CPOL/CPHA combinations);
  default \texttt{0}.
\item[\texttt{Clock\_Hz}] bit clock in Hz; default \texttt{1\_000\_000}, clamped to
  roughly 80\,kHz\,..\,80\,MHz.
\item[\texttt{Sclk}, \texttt{Mosi}, \texttt{Miso}, \texttt{Cs}] the four lines as
  \texttt{Optional\_Pin}; \texttt{Cs} is the host's single hardware chip-select and
  defaults to \texttt{No\_Pin} --- with more than one device on the bus, give each
  device its own select via \texttt{CS\_Pin} at \texttt{Acquire} instead (below).
...
\begin{lstlisting}
   ...
   Acquire (S, SPI2, CS_Pin => 10);    --  driver drives IO10 active-low
