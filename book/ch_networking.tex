...
network. \texttt{FTP\_Server.Run} takes a mounted \texttt{ESP32S3.Ext4.FS.Mount}
and runs the accept loop --- one client at a time, a control connection plus one
passive data connection per transfer:

\begin{lstlisting}
procedure Run (FS          : not null access ESP32S3.Ext4.FS.Mount;
               Local_IP    : String;                       -- advertised in PASV
               Server_Name : String := "Ada ESP32-S3";     -- the 220 greeting
               Port        : Port_Type := 21;
               Data_Port   : Port_Type := 50_000;
               Read_Only   : Boolean := False);
\end{lstlisting}

The server side of the socket API is \texttt{Bind\_Socket} / \texttt{Listen\_%
Socket} / \texttt{Accept\_Socket} (the echo example, \S\ref{ch:net:examples}); each
FTP command then maps onto an ext4 operation --- \texttt{LIST} onto
\texttt{Iterate} (plus \texttt{Stat} for the \texttt{ls -l} columns),
\texttt{RETR} onto the offset \texttt{Read\_File}, \texttt{STOR} onto
\texttt{Create\_File} + \texttt{Append} + \texttt{Commit}, \texttt{DELE} /
\texttt{MKD} / \texttt{RMD} onto \texttt{Unlink} / \texttt{Mkdir} / \texttt{Rmdir}.
The filesystem's \texttt{Ada.IO\_Exceptions} are caught and turned into FTP
\texttt{550} replies, so a bad request never takes the server down; \texttt{Read\_%
Only} refuses the mutating commands. A desktop client --- \texttt{ftp}, FileZilla,
a browser, Python \texttt{ftplib} --- can then browse, download, upload and manage
the board's flash.
...
fresh ext4 on the W25Q flash, then serves it; HW-verified against \texttt{ftplib}
(login, \texttt{LIST}, \texttt{RETR}/\texttt{SIZE}, \texttt{STOR} with a byte-exact
read-back, \texttt{MKD}, \texttt{DELE}).
