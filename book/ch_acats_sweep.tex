...
\section{Two ways to run the suite}
...
\section{Getting the grade off the chip}
...
\section{The grader must not trust the test id}
...
\section{Phase 1 --- parallel build}
...
\section{Phase 2 --- parallel run and grade}
...
\textbf{Outcome} & \textbf{Count} & \textbf{Meaning} \\ \hline
\endhead
...
\subsection{A wider net: the maximal list across seven boards}
...
the interactive interrupt and suspension tests that need a bench-generated
interrupt, and \texttt{C96004}'s post-2099 Calendar range --- no crashes, and no
surprise failures in the conformant core.

\section{The embedded profile}
...
\section{Closing a won't-build category: the Ada.Calendar family}
...
tests \texttt{C96005}/\texttt{C96006} PASS. The one Calendar test that does \emph{not} pass is
\texttt{C96004}, which requires \texttt{Year\_Number'Last = 2399} (Ada~2005); the bare
runtime's Julian-day Calendar is bounded to the Ada~95 \texttt{2099}, and lifting it needs a
new time base rather than a subtype edit (see the Limitations chapter). \texttt{light-tasking} is deliberately
