...
would take a coprocessor-disabled exception. The bare runtime's \texttt{start.S}
enables both by writing \texttt{CPENABLE = 16\#09\#} -- bit~0 for the FPU, bit~3 for
PIE (Chapter~\ref{ch:bootloader}); the value is exactly the \texttt{XCHAL\_CP\_MASK}
the S3 core configuration declares. One word at boot, and SIMD code runs.
...
1024-element vectors, runs each SIMD op against a scalar Ada reference to confirm the
results agree, then brackets both with \texttt{Cycles} (the \texttt{rsr.ccount} reader
above) and prints the speed-up. It builds into a flashable image through the normal
bare-metal path -- the SIMD library is a library like any other
(\texttt{libs/esp32s3\_simd}), pulled in with one \texttt{with} line in the project
file.

The upstream project published a full benchmark run on real silicon (1024 elements,
64 iterations, \texttt{-O3}); a representative slice gives the shape of the win:

\begin{center}
\begin{tabular}{l r r r}
\toprule
\textbf{Operation} & \textbf{SIMD (\textmu s)} & \textbf{Scalar (\textmu s)} & \textbf{Speed-up} \\ \midrule
\texttt{Add} i8           &  171 & 6987 & 40.9$\times$ \\
\texttt{Add} i16          &  278 & 6578 & 23.7$\times$ \\
\texttt{Add} i32          &  476 & 2877 &  6.0$\times$ \\
\texttt{Add} f32          & 1096 & 3704 &  3.4$\times$ \\
\texttt{Dot\_Product} i8  &  131 & 3297 & 25.2$\times$ \\
\texttt{Max} i8           &  166 & 3698 & 22.3$\times$ \\
\bottomrule
\end{tabular}
\end{center}

The pattern is the one the width predicts: the narrower the element, the more lanes
per 128-bit register, the larger the win -- byte operations, with sixteen lanes,
run an order of magnitude faster, while \texttt{Integer\_32} and \texttt{Float\_32}
at four lanes land in the low single digits. These are the upstream's numbers; the
on-board example here is built to reproduce the measurement on your own board.
...
version-controlled like everything else -- and the one genuinely hard part, on a
chip whose toolchain does not target its own SIMD unit, was teaching the assembler
the opcodes and turning the coprocessor on, both of which are now one line each.
