...
\begin{center}
\begin{tabular}{l l l}
\textbf{Window} & \textbf{Range} & \textbf{Action} \\ \hline
...
External PSRAM is the one place the project still leans on opaque Espressif
ROM/blob routines, and for a good reason: bringing up octal PSRAM reconfigures
the very MSPI timing that flash execute-in-place depends on, so it has to run
from RAM, carefully, at exactly the right moment. The 2nd-stage bootloader is
that moment. The bring-up (a small C shim, \texttt{psram\_boot.c}) does:
...
\item \textbf{Wire the octal pins.} \texttt{esp\_rom\_opiflash\_pin\_config} and
  ...
  mode needs.
\item \textbf{Enable and size the PSRAM} via the octal-PSRAM ROM implementation,
  reporting the detected size.
\item \textbf{Force the cache read mode.} The tuning step leaves the SPI0 data-in
  mode wrong for OPI-DDR cache reads; a single register poke
  (\texttt{16\#6000\_30C0\#}) forces the correct mode.
...
\begin{center}
\begin{tabular}{l l l}
\textbf{Region} & \textbf{Base} & \textbf{What lives there} \\ \hline
