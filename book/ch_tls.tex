\chapter{Pure-Ada TLS 1.3: HTTPS without a C library}
\label{ch:tls}
...
two things already in the box: \textbf{SPARKNaCl} (a vendored, SPARK-proven Ada
...
\section{The shape of the stack}
\label{ch:tls:stack}
...
\subsection{ClientHello: offering what we can do}
...
\subsection{Two ways to agree on a secret}
\label{ch:tls:ecdhe}
...
\subsection{Checking the signatures}
...
accelerator and comparing the recovered padding block --- PKCS\#1~v1.5 for chain
links, PSS for the TLS \texttt{CertificateVerify}. \textbf{ECDSA/P-256}
certificates are verified entirely in software with \texttt{P256.Verify}. The
\texttt{CertificateVerify} step, where the server signs the handshake transcript
to prove key possession, accepts either \texttt{rsa\_pss\_rsae\_sha256} or
\texttt{ecdsa\_secp256r1\_sha256}; the chain verifier dispatches each link on how
that certificate was signed. The result is a client that authenticates both an
RSA server (Let's Encrypt issues these) and an ECDSA server (Cloudflare, Google,
many CDNs) --- both confirmed against the real sites.

\subsection{Anchoring the chain --- and knowing the time}
...
\section{The application channel, and a real example}
\label{ch:tls:app}
...
authenticating it on the way up (skipping the post-handshake session tickets a
server may send). To the application it is just a reliable byte channel --- a GET
goes in, a JSON reply comes out.
...
and RSA (PKCS\#1 and PSS, SHA-256) and ECDSA-P256 (SHA-256/384) certificate
verification, with chain validation to pinned roots and NTP-based validity. That
is enough to fetch from the large majority of real public HTTPS servers, as the
open-meteo and Cloudflare runs show.

It deliberately stops short of a few things a production stack would add: the
\texttt{TLS\_AES\_256\_GCM\_SHA384} and ChaCha20-Poly1305 suites (the first needs
SHA-384 wired through the schedule, the second a ChaCha20 SPARKNaCl lacks);
certificate \emph{policy} checks beyond the signature chain (basic-constraints
CA flags, key-usage, path-length); certificate revocation (OCSP/CRL); session
resumption; and client certificates. None of these are hard in principle --- each
is a bounded slice on top of the same primitives --- but each is a place where
``it compiled'' is not the same as ``it is safe,'' which is exactly why they are
called out here rather than quietly assumed.
