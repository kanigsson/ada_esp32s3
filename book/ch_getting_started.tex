...
The repository ships with around forty ready-to-run examples, and one script at
...
\section{Your own project, out of tree: \texttt{esp32-ada}}
\label{sec:esp32-ada}
...
\texttt{esp32-ada install-vim} & Symlink the Vim/Neovim plugin. \\ \hline
\end{longtable}
...
\subsection{Taking an example out of tree to edit}
...
esp32-ada build -P embedded                             # match the example's profile
...
\item \textbf{Bring board config and C natives if customized.} If the example sets
  ...
  \texttt{esp32-ada config}); if it has C natives, copy its \texttt{main/glue.c}.
