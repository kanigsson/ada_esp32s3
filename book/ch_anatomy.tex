...
\textbf{Case is insignificant --- so you pick a convention and the compiler keeps you
honest.} Ada folds letter case for identifiers and reserved words alike:
\texttt{Led}, \texttt{led} and \texttt{LED} are one name, and \texttt{LOOP},
\texttt{Loop} and \texttt{loop} are one keyword. Crucially, the language also forbids
two names that differ \emph{only} in case --- precisely so a reader never has to
squint at \texttt{Value} versus \texttt{value} to tell two things apart. The freedom
is therefore really a discipline: choose a house style and the consistency carries
meaning. This book uses the conventional Ada style --- \texttt{Mixed\_Case} for
identifiers and types (\texttt{Pin\_Id}, \texttt{Clock}), lower case for reserved
words (\texttt{procedure}, \texttt{loop}, \texttt{end}) --- so when you see that shape
you are reading a choice, not a rule the compiler imposed.
