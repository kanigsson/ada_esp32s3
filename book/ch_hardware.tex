...
\texttt{ESP32S3\_Registers.*} (one package per peripheral). Each register is a
record with a representation clause, and each peripheral is one big record placed
at its base address. A driver then writes fields by name:
...
GPIO_Periph.OUT_W1TS.OUT_W1TS := 2 ** 2;   --  set GPIO2, by field name
\end{lstlisting}
...
\centering
\begin{tabular}{@{}l l l@{}}
\textbf{Base} & \textbf{Top} & \textbf{Region} \\ \hline
