...
register read returns. In most languages the reach-for tool is a closure -- a
nested subprogram that captures the surrounding state. That is exactly what this
platform forbids. A subprogram that captures its environment compiles to a
\emph{trampoline} -- a scrap of executable code GCC writes onto the stack -- and on
