...
--  BARE-BOARD FULL profile: the embedded/Jorvik base ships the RESTRICTED
--  Ada.Interrupts body (No_Dynamic_Attachment) whose dynamic operations all
--  raise Program_Error.  The full profile lifts that restriction, so use this
--  delegating body that forwards to System.Interrupts (the bare-board full
--  s-interr in this overlay).  Static pragma Attach_Handler bypasses this unit
--  (the compiler calls Install_Handlers directly); these are the DYNAMIC ops
--  (Ada.Interrupts.Attach_Handler/Detach_Handler/Exchange_Handler/...).
