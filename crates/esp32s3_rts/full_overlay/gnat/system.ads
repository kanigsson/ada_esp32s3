...
--  No_Exception_Registration is intentionally NOT set in this (full) profile:
--  System.Exception_Table is present, so name registration works and
...
--
--  Unlike light-tasking/embedded (which use `pragma Profile (Jorvik)`), this
--  profile sets NO profile pragma, so GNAT selects the FULL tasking runtime
--  (System.Tasking.Stages etc.) rather than the restricted one.  That runtime
--  is being ported here from the version-matched host GNAT onto the bareboard
--  BB kernel; until the port lands, this profile DOES NOT BUILD.  See
--  crates/esp32s3_rts/full_overlay/README.md for the plan,
--  the porting surface, and the milestones (M1..M5).
--
--  Only the dispatching/locking policies are set (sensible real-time
--  defaults); no restrictions are applied -- full Ada tasking is the goal.
--  =======================================================================
